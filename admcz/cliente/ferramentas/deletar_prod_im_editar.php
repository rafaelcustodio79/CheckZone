...
if ($idIM === null || $invoice === null) {
    header('Location: ../../cliente.php?a=ferramentas&b=invoice_maker_listar&flag=erro&tip=ID da Invoice não fornecida.');
    exit;
}
...
if ($idProd && $idIM && $invoice) {
    ...
    if ($stmt->execute()) {
        ...
        header('Location: ../../cliente.php?a=ferramentas&b=invoice_maker_editar&idIM='.$idIM.'&invoice='.$invoice.'&flag=success&tip=Produto excluído com sucesso!&tabAtiva=2');
        exit();
    } else {
        header('Location: ../../cliente.php?a=ferramentas&b=invoice_maker_editar&idIM='.$idIM.'&invoice='.$invoice.'flag=erro&tip=Nenhuma informação foi encontrada. Reinicie o processo!');
        exit();
    }
} else {
    header('Location: ../../cliente.php?a=ferramentas&binvoice_maker_editar&idIM='.$idIM.'&invoice='.$invoice.'&flag=erro&tip=IDs inválido.');
    exit();
}