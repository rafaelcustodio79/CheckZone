...
if ($idIM) {
    ...
    try {
        ...
        header('Location: ../ferramentas.php?a=ferramentas&b=invoice_maker_editar&idIM='.$idIM.'&invoice='.$invoice_number.'&flag=success&tip=Dados da Invoice atualizados com sucesso!&tabAtiva=2');
        exit();
    } catch (PDOException $e) {
        // Redirecionamento em caso de erro
        header('Location: ../ferramentas.php?a=ferramentas&b=invoice_maker_editar&idIM='.$idIM.'&invoice='.$invoice_number.'&flag=erro&tip=Não foi possível realizar a operação. '.$e->getMessage());
    }
} else {
    // Lidar com o caso de não ter o ID do registro para atualizar
    header('Location: ../ferramentas.php?a=ferramentas&b=invoice_maker_editar&flag=erro&tip=ID do registro não encontrado.');
    exit();
}