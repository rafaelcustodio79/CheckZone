...
if ($idIM) {
    ...
    try {
        ...
        header('Location: ../cliente.php?pg=invoice_maker_editar&idIM='.urlencode($idIM).'&invoice='.urlencode($invoice_number).'&flag=success&tip=Dados do Bank atualizados com sucesso!');
        ob_end_flush(); // Limpar e enviar o buffer
        exit();
    } catch (PDOException $e) {
        // Redirecionamento em caso de erro
        ob_start();
        header('Location: ../cliente.php?pg=invoice_maker_editar&idIM='.urlencode($idIM).'&invoice='.urlencode($invoice_number).'&flag=erro&tip=Não foi possível realizar a operação. '.urlencode($e->getMessage()));
        ob_end_flush();
        exit();
    }
} else {
    // Caso não haja ID para atualização
    header('Location: ../cliente.php?pg=invoice_maker_editar&flag=erro&tip=ID do registro não encontrado.');
    exit();
}
