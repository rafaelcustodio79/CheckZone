...
if ($idIM === null || $invoice === null) {
    header('Location: ../ferramentas.php?a=ferramentas&b=listar_invoice_maker&flag=erro&tip=ID da Invoice não fornecida.');
    exit;
}
...
if ($idProd && $idIM && $invoice) {
    ...
    if ($stmt->execute()) {
        ...
        header('Location: ../ferramentas.php?a=ferramentas&b=invoice_maker&idIM='.$idIM.'&invoice='.$invoice.'&flag=success&tip=Produto excluído com sucesso!&tabAtiva=2');
        exit();
    } else {
        header('Location: ../ferramentas.php?a=ferramentas&b=invoice_maker&idIM='.$idIM.'&invoice='.$invoice.'flag=erro&tip=Nenhuma informação foi encontrada. Reinicie o processo!');
        exit();
    }
} else {
    header('Location: ../ferramentas.php?a=ferramentas&b=invoice_maker&idIM='.$idIM.'&invoice='.$invoice.'&flag=erro&tip=IDs inválido.');
    exit();
}